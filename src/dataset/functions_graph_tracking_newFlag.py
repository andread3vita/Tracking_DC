...
def create_inputs_from_table(output, get_vtx, cld=False, tau=False):
    ...
    isProducedBySecondary = np.int32(output["pf_mask"][2,0:number_hits])
    particles_pt = torch.tensor(output["pf_vectors"][6,0:number_hits])
...
def assign_noise(hit_particle_link, isProducedBySecondary, particles_pt, pt_thr= 0.1):
    """
    Assign noise to the hits that are not produced by secondary particles
    """
    
    unique_p_numbers = torch.unique(hit_particle_link)
    mask_noise_hit = (torch.from_numpy(isProducedBySecondary) == 1) #| (particles_pt < pt_thr)

  
    list_remove = torch.unique(hit_particle_link[mask_noise_hit])
    if len(list_remove) > 0:
        mask = torch.tensor(
            np.full((len(hit_particle_link)), False, dtype=bool)
        )
    ...
    if len(list_remove) > 0:
        ...
        mask_particles = torch.tensor(
            np.full((len(list_p)), False, dtype=bool)
        )

    return mask.bool(), ~torch.tensor(mask_particles).bool()
...
def create_graph_tracking_global(output, get_vtx=False, vector=False, tau=False, overlay=False):
    ...
    if len(result) == 1:
        ...
        if not overlay:
            ...
            # I am assigning noise to the hits that are produced by secondary particles, but I could also remove them. What should I do?
            mask_noise, mask_particles = assign_noise(hit_particle_link, isProducedBySecondary,particles_pt,0.1)
            hit_particle_link[mask_noise] = -1
            y_data_graph = y_data_graph[mask_particles]
            cluster_id, unique_list_particles = find_cluster_id(hit_particle_link)    
            

            # hit_type_one_hot = hit_type_one_hot[~mask_noise]
            # cluster_id = cluster_id[~mask_noise]
            # hit_particle_link = hit_particle_link[~mask_noise]
            # if tau:
            #     hit_particle_link_tau = hit_particle_link_tau[~mask_noise]
            # features_hits = features_hits[~mask_noise]
            # hit_type = hit_type[~mask_noise]
            # y_data_graph = y_data_graph[mask_particles]
            # cluster_id, unique_list_particles = find_cluster_id(hit_particle_link)
...
def remove_loopers(hit_particle_link, y, coord, cluster_id):
    ...
    print(coord.shape, cluster_id.shape, hit_particle_link.shape, mask_all.shape,unique_p_numbers.shape)
